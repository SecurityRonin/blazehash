...
fn test_intersect_finds_common_paths() {
    ...
    assert!(
        ...
        "second shared path must appear"
    );
    assert!(
        !stdout.contains("only_in_a.txt"),
        "a-only path must not appear"
    );
    assert!(
...
fn test_intersect_uses_hash_from_first_manifest() {
    ...
    assert!(
        !stdout.contains("dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd"),
...
fn test_intersect_no_common_paths_exits_nonzero() {
    ...
    assert!(
        !out.status.success(),
        "empty intersection must exit nonzero"
    );
}
