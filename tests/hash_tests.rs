...
fn hash_file_streaming_matches_mmap() {
    ...
    small.flush().unwrap();
    let streaming_result = hash_file(
        small.path(),
        &[Algorithm::Sha256],
        false,
        false,
...
fn cli_entropy_flag_shows_entropy_in_csv() {
    ...
    let output = Command::cargo_bin("blazehash")
        .unwrap()
        .args(["--entropy", "--format", "csv", f.path().to_str().unwrap()])
        .output()
        .unwrap();
    ...
    assert_eq!(
        fields.len(),
        4,
        "expected 4 CSV fields: size,blake3,entropy,filename; got: {data_row}"
    );
