...
mod qr_tests {
    ...
    fn test_build_qr_content_minimal() {
        ...
        assert!(content.starts_with("BLAZEHASH:sha256="), "must start with BLAZEHASH:sha256=");
        assert!(content.len() < 300, "minimal content must fit in QR: len={}", content.len());
    }
    ...
    fn test_build_qr_content_with_case() {
        let manifest = write_temp_manifest("%%blazehash-1.0\n## case: CASE-2026-001\n## examiner: Jane Smith\nsha256,path\nabc,/foo\n");
        let content = build_qr_content(manifest.path(), None, None).unwrap();
        assert!(content.contains("case=CASE-2026-001"), "must include case id: {content}");
        assert!(content.contains("examiner=Jane+Smith") || content.contains("examiner=Jane%20Smith") || content.contains("examiner=Jane Smith"), "must include examiner: {content}");
    }
