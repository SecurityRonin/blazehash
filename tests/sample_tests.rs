...
fn test_sample_count_limits_output() {
    ...
    assert_eq!(
        data.len(),
        5,
        "sample --count 5 must return exactly 5 entries"
    );
}
...
fn test_sample_count_larger_than_manifest_returns_all() {
    ...
    assert_eq!(
        data.len(),
        20,
        "requesting more than available must return all 20 entries"
    );
}
...
fn test_sample_entries_are_valid_manifest_lines() {
        ...
        .filter(|l| !l.trim().is_empty() && !l.trim().starts_with('#'))
    {
        ...
        assert_eq!(
            parts.len(),
            3,
            "sampled entry must be valid manifest line: {line}"
        );
    }
}
