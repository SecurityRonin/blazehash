...
fn test_sym_diff_finds_a_only_entries() {
    ...
    assert!(
        stdout.contains("/a/file1.txt"),
        "/a/file1.txt must appear (A-only)"
    );
    assert!(
        stdout.contains("/a/file2.txt"),
        "/a/file2.txt must appear (A-only)"
    );
}
...
fn test_sym_diff_finds_b_only_entries() {
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("/b/file3.txt"),
        "/b/file3.txt must appear (B-only)"
    );
    assert!(
        stdout.contains("/b/file4.txt"),
        "/b/file4.txt must appear (B-only)"
    );
}
...
fn test_sym_diff_excludes_shared_entries() {
    ...
    assert!(
        ...
        "/shared.txt must NOT appear (in both manifests)"
    );
}
...
fn test_sym_diff_missing_manifest_fails() {
    ...
    assert!(
        !out.status.success(),
        "sym-diff must fail when first manifest is missing"
    );
}
...
fn test_sym_diff_output_to_file() {
    ...
    assert!(
        content.contains("/a/file1.txt"),
        "output file must contain A-only entry"
    );
    assert!(
        content.contains("/a/file2.txt"),
        "output file must contain second A-only entry"
    );
    assert!(
        content.contains("/b/file3.txt"),
        "output file must contain B-only entry"
    );
    assert!(
        content.contains("/b/file4.txt"),
        "output file must contain second B-only entry"
    );
    assert!(
        !content.contains("/shared.txt"),
        "output file must NOT contain shared entry"
    );
}
