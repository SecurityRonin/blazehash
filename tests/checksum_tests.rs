...
fn test_checksum_is_deterministic() {
    ...
    fs::write(&p, "blake3  aaaa  file.txt\n").unwrap();
    let r1 = Command::cargo_bin("blazehash").unwrap()
        .args(["checksum", p.to_str().unwrap()]).output().unwrap().stdout;
    let r2 = Command::cargo_bin("blazehash").unwrap()
        .args(["checksum", p.to_str().unwrap()]).output().unwrap().stdout;
    assert_eq!(r1, r2);
}
...
fn test_checksum_changes_when_file_changes() {
    ...
    let p = dir.path().join("m.hash");
    fs::write(&p, "blake3  aaaa  file.txt\n").unwrap();
    let h1 = Command::cargo_bin("blazehash").unwrap()
        .args(["checksum", p.to_str().unwrap()]).output().unwrap().stdout;
    fs::write(&p, "blake3  bbbb  file.txt\n").unwrap();
    let h2 = Command::cargo_bin("blazehash").unwrap()
        .args(["checksum", p.to_str().unwrap()]).output().unwrap().stdout;
    assert_ne!(h1, h2);
}
